...
#[Name('travel-server')]
#[Version('1.0.0')]
#[Instructions('Provides authenticated trip, package, and hotel search tools for the travel platform. Every tool forwards the caller bearer token to the remote TravelBooking API, the overview tool can fetch all three datasets together for a single lookup term, and the travel intent tool uses the configured LLM to normalize customer messages into a search term, optional destination, requested resource types, and question focus. When presenting tool results to the user, answer the actual question first, then prefer attractive and professional HTML with clear section headings, a short natural-language summary, result counts such as "There are 5 trips", and numbered detail blocks like "Trip 1". Use tables for quick comparison when helpful, but keep the overall response conversational and human-readable rather than dumping raw JSON or flat field lists.')]
